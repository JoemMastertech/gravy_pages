import i from"./AuthService.js";import"./index.js";class h{constructor(){this.container=document.createElement("div"),this.container.id="auth-modal-container",this.container.className="login-modal-overlay hidden",this.render(),this.bindEvents(),document.body.appendChild(this.container)}render(){this.container.innerHTML=`
    ...
    `,document.head.appendChild(t)}bindEvents(){i.subscribe(t=>{console.log("[LoginModal] Auth State Updated:",t==null?void 0:t.email),this.updateView()}),this.container.querySelector(".close-btn").addEventListener("click",()=>this.hide()),this.container.querySelector("#btn-login-google").addEventListener("click",async()=>{try{console.log("[LoginModal] Google Login Clicked");const t=window.container||window.DIContainer;if(t)try{const o=t.resolve("AuthRepository");console.log("[LoginModal] AuthRepository resolved:",o),o&&typeof o.getRedirectUrl=="function"&&console.log("[LoginModal] Target redirectUrl from repo:",o.getRedirectUrl())}catch(o){console.error("[LoginModal] Error resolving AuthRepository:",o)}await i.loginWithGoogle()}catch(t){this.showMessage(t.message,"error")}}),this.container.querySelector("#login-form-email").addEventListener("submit",async t=>{t.preventDefault();const o=this.container.querySelector("#input-email").value,e=this.container.querySelector(".btn-email");e.setAttribute("data-translate","true"),e.setAttribute("data-key","auth-email-sending"),e.textContent="Enviando...",e.disabled=!0;try{await i.loginWithEmail(o),this.showMessage("¡Enlace enviado! Revisa tu correo.","success"),e.setAttribute("data-translate","true"),e.setAttribute("data-key","auth-email-success"),e.textContent="Revisa tu correo"}catch(a){this.showMessage(a.message,"error"),e.setAttribute("data-translate","true"),e.setAttribute("data-key","auth-submit-btn"),e.textContent="Enviar Enlace Mágico",e.disabled=!1}}),this.container.querySelector("#btn-logout").addEventListener("click",async()=>{console.log("[LoginModal] Logout Button Clicked");try{const t=this.container.querySelector("#btn-logout");t.setAttribute("data-translate","true"),t.setAttribute("data-key","auth-logout-sending"),t.textContent="Cerrando sesión...",console.log("[LoginModal] Calling AuthService.logout()..."),await i.logout(),console.log("[LoginModal] AuthService.logout() returned."),this.hide()}catch(t){console.error("[LoginModal] Logout Error:",t);const o=this.container.querySelector("#btn-logout");o.setAttribute("data-translate","true"),o.setAttribute("data-key","auth-logout-error"),o.textContent="Error (Reintentar)",alert("Error al cerrar sesión: "+t.message)}})}showMessage(t,o){const e=this.container.querySelector("#login-message");e.textContent=t,e.className=`login-message ${o}`,e.classList.remove("hidden")}show(){this.setMode("login"),this.updateView(),this.container.classList.remove("hidden")}showForRegistration(){this.setMode("register"),this.updateView(),this.container.classList.remove("hidden"),i.setRegistrationMode(!0)}setMode(t){const o=this.container.querySelector("#modal-title"),e=this.container.querySelector("#modal-subtitle"),a=this.container.querySelector("#btn-login-google");t==="register"?(o.setAttribute("data-translate","true"),o.setAttribute("data-key","auth-register-title"),o.textContent="Registrar Nuevo Negocio",e.setAttribute("data-translate","true"),e.setAttribute("data-key","auth-register-subtitle"),e.textContent="Crea tu cuenta de administrador en 1 paso.",a.innerHTML=`
                <svg width="18" height="18" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg">
                    <path d="M17.64 9.2c0-.637-.057-1.251-.164-1.84H9v3.481h4.844c-.209 1.125-.843 2.078-1.796 2.717v2.258h2.908c1.702-1.567 2.684-3.874 2.684-6.615z" fill="#4285F4"/>
                    <path d="M9 18c2.43 0 4.467-.806 5.956-2.18L12.049 13.56c-.806.54-1.836.86-3.049.86-2.344 0-4.328-1.584-5.036-3.715H.957v2.332A8.997 8.997 0 0 0 9 18z" fill="#34A853"/>
                    <path d="M3.964 10.71A5.41 5.41 0 0 1 3.682 9c0-.593.102-1.17.282-1.71V4.958H.957A8.996 8.996 0 0 0 0 9c0 1.452.348 2.827.957 4.042l3.007-2.332z" fill="#FBBC05"/>
                ...
                <span data-translate="true" data-key="auth-google-btn">Continuar con Google</span>`)}hide(){this.container.classList.add("hidden")}updateView(){const t=i.user,o=this.container.querySelector("#view-guest"),e=this.container.querySelector("#view-auth");if(t){o.classList.add("hidden"),e.classList.remove("hidden");const a=this.container.querySelector("#profile-name"),g=this.container.querySelector("#profile-email"),r=this.container.querySelector("#profile-img"),n=this.container.querySelector("#profile-initial"),s=this.container.querySelector("#btn-logout");s.setAttribute("data-translate","true"),s.setAttribute("data-key","auth-logout-btn"),s.textContent="Cerrar Sesión";const l=t.email,d=t.user_metadata||{},c=d.full_name||l.split("@")[0],u=d.avatar_url;a.textContent=c,g.textContent=l,u?(r.src=u,r.style.display="block",n.classList.add("hidden")):(r.style.display="none",n.textContent=c.charAt(0).toUpperCase(),n.classList.remove("hidden"))}else o.classList.remove("hidden"),e.classList.add("hidden"),this.container.querySelector("#login-message").classList.add("hidden")}}const v=new h;export{v as default};
